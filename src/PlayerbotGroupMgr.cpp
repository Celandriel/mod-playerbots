    ...
    bool PlayerbotGroupMgr::IsCompositionAvailable(const TargetGroupComposition& composition)
    {
        ...
        if (botRole == BOT_ROLE_TANK && tanksNeeded > 0 ) tanksNeeded--;
        else if (botRole == BOT_ROLE_HEALER && healersNeeded > 0) healersNeeded--;
        else if (botRole == BOT_ROLE_DPS && dpsNeeded > 0) dpsNeeded--;

        if (roleCounts[BOT_ROLE_TANK] < tanksNeeded) return false;
        if (roleCounts[BOT_ROLE_HEALER] < healersNeeded) return false;
        if (roleCounts[BOT_ROLE_DPS] < dpsNeeded) return false;
